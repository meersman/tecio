#!/usr/bin/env python3
"""pytest tests for :class:`tecio.szl.Read` -- SZL high-level reader.

Pattern per test:
    1. Open an existing or hand-authored ``.szplt`` file via tecio.open(..., "r")
    2. Assert on metadata, container types (ZoneList / VariableList), and
       variable values

Run directly:

    $ python tests/test_szl_read.py -v

Keep output files for Tecplot 360 inspection:

    $ python tests/test_szl_read.py -v --keep-files
"""
...
class TestReadDump:
    """Dump every header and data field for the shared Onera SZL fixture."""

    def test_read_dump(self, onera_szplt: Path) -> None:
        """Print out all headers and data fields for the Onera SZL fixture.
        ...
        szl = tecio.open(str(onera_szplt), "r")
...
class TestContainers:
    ...
    def test_zone_and_variable_containers(self, onera_szplt: Path) -> None:
        ...
        assert isinstance(szl.zone, tecio.ZoneList)
        assert len(szl.zone) == szl.num_zones

        if szl.num_zones >= 2:
            sub = szl.zone[0:2]
            assert isinstance(sub, tecio.ZoneList)
            assert len(sub) == 2
            assert sub[0].title == szl.zone[0].title
            assert sub[1].title == szl.zone[1].title

        zone = szl.zone[0]

        assert isinstance(zone.variable, tecio.VariableList)
        assert len(zone.variable) == szl.num_vars
        assert zone.variable.names() == szl.variables

        first_name = szl.variables[0]
...
class TestGetArray:
    ...
    def test_get_array_matches_variable_values(self, onera_szplt: Path) -> None:
        ...
        with pytest.raises(IndexError):
            zone.get_array(szl.num_vars + 10)

    def test_get_array_passive_round_trip(self, output_path: Callable) -> None:
        """Round-trip a tiny SZL file with one passive variable.
        ...
        path = output_path("test_szl_get_array_passive.szplt")
