...
def example_compute_mach():
    ...
    for i, zone in enumerate(data.zones):
        ...
        print(
            f"  Zone {i+1}: Mach range [{mach_data.min():.3f}, {mach_data.max():.3f}]"
        )
...
def example_create_from_scratch():
    ...
    zone = data.add_zone(
        title="Structured Grid", zone_type=ZoneType.ORDERED, dimensions=(10, 10, 10)
    )
...
def example_partial_load():
    ...
    data = TecData.from_szl_file(
        "large_file.szplt", load_data=True, zones=[1, 2, 3], variables=[1, 2, 5]
    )
...
def example_velocity_magnitude():
    ...
    vmag_idx = data.compute_magnitude(
        (u_idx, v_idx, w_idx), result_name="Velocity_Magnitude"
    )
